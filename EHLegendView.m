...
- (void)drawRect:(CGRect)rect {
	...
	for (int i = 0; i < self.titles.count; i++) {
		...
		EH_DRAW_TEXT_AT_POINT(title, CGPointMake(COLOR_PADDING + PADDING, y + PADDING), self.legendFont);
...
- (CGSize)sizeThatFits:(CGSize)size {
	...
	for (NSString *title in self.titles) {
		CGSize s = EH_TEXT_SIZE(title, self.legendFont);
		w = MAX(w, s.width);
	}
