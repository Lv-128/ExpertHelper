...
#pragma mark - EHRevealView Class

@interface EHRevealView: UIView
{
    __weak EHRevealViewController *_c;
}
...
@interface EHRevealViewController()
...
- (id)initWithFrame:(CGRect)frame controller:(EHRevealViewController*)controller
{
...
{
    __weak EHRevealViewController *_revealVC;
    UIView *_view;
...
- (id)initWithRevealController:(EHRevealViewController*)revealVC containerView:(UIView*)view fromVC:(UIViewController*)fromVC
...
@interface EHRevealViewController()<UIGestureRecognizerDelegate>
{
    EHRevealView *_contentView;
...
@implementation EHRevealViewController
{
...
- (void)loadView
{
    ...
    _contentView = [[EHRevealView alloc] initWithFrame:frame controller:self];
...
- (void)_dispatchSetFrontViewPosition:(FrontViewPosition)frontViewPosition animated:(BOOL)animated
{
    ...
    __weak EHRevealViewController *theSelf = self;
...
{
    [self _performTransitionToViewController:newRearViewController operation:EHRevealControllerOperationReplaceRearController animated:animated];
}
...
{
    [self _performTransitionToViewController:newFrontViewController operation:EHRevealControllerOperationReplaceFrontController animated:animated];
}
...
{
    [self _performTransitionToViewController:newRightViewController operation:EHRevealControllerOperationReplaceRightController animated:animated];
}


// Primitive method for animated controller transition
- (void)_performTransitionToViewController:(UIViewController*)new operation:(EHRevealControllerOperation)operation animated:(BOOL)animated
{
    ...
    if ( operation == EHRevealControllerOperationReplaceRearController )
        old = _rearViewController, _rearViewController = new, view = _contentView.rearView;
    
    else if ( operation == EHRevealControllerOperationReplaceFrontController )
        old = _frontViewController, _frontViewController = new, view = _contentView.frontView;
    
    else if ( operation == EHRevealControllerOperationReplaceRightController )
...
- (void)prepareForSegue:(EHRevealViewControllerSegue *)segue sender:(id)sender   // TO REMOVE: DEPRECATED IMPLEMENTATION
{
    // This method is required for compatibility with EHRevealViewControllerSegue, now deprecated.
    // It can be simply removed when using EHRevealViewControllerSegueSetController and EHRevealViewControlerSeguePushController
    
    NSString *identifier = segue.identifier;
    if ( [segue isKindOfClass:[EHRevealViewControllerSegue class]] && sender == nil )
    {
        if ( [identifier isEqualToString:SWSegueRearIdentifier] )
        {
            segue.performBlock = ^(EHRevealViewControllerSegue* rvc_segue, UIViewController* svc, UIViewController* dvc)
            {
...
@implementation UIViewController(EHRevealViewController)

- (EHRevealViewController*)revealViewController
{
    UIViewController *parent = self;
    Class revealClass = [EHRevealViewController class];
...
#pragma mark - EHRevealViewControllerSegueSetController segue identifiers
...
#pragma mark - EHRevealViewControllerSegueSetController class

@implementation EHRevealViewControllerSegueSetController

- (void)perform
{
    NSString *identifier = self.identifier;
    EHRevealViewController *rvc = self.sourceViewController;
...
#pragma mark - EHRevealViewControllerSeguePushController class

@implementation EHRevealViewControllerSeguePushController

- (void)perform
{
    EHRevealViewController *rvc = [self.sourceViewController revealViewController];
    UIViewController *dvc = self.destinationViewController;
...
#pragma mark - EHRevealViewControllerSegue Class

@implementation EHRevealViewControllerSegue  // DEPRECATED
